...
GURL GetUpdateURL(
    ...
    std::unique_ptr<serp_metrics::SerpMetricsAggregator>
  ...
  return stats_updater_params.GetUpdateURL(
      ...
      std::move(serp_metrics_aggregator));
}
...
std::unique_ptr<serp_metrics::SerpMetricsAggregator> GetSerpMetricsAggregator(
...
void BraveStatsUpdater::SendServerPing() {
  ...
  resource_request->url =
      GetUpdateURL(endpoint, *stats_updater_params,
                   GetSerpMetricsAggregator(pref_service_, profile_manager_));
