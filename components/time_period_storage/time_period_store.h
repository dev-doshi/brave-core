...
class TimePeriodStore {
 public:
  ...
  // Saves a list of time period values.
  virtual void Save(base::ListValue data) = 0;

  // Returns a pointer to a list of time period values. Returns nullptr if no
  // values are stored. Returned pointer shouldn't be cached because it may be
  // invalidated by the time it's used.
  virtual const base::ListValue* Get() const = 0;
