...
TEST_F(LocalAIServiceTest, InFlightRequestsDrainedOnDestroyed) {
  BindFakeModelWorker();

  base::test::TestFuture<const std::vector<double>&> future;
  service_->GenerateEmbeddings("in-flight", future.GetCallback());

  // Destroy background contents before the Mojo response arrives.
  ASSERT_TRUE(last_created_web_ui_);
  last_created_web_ui_->SimulateDestroyed();

  // In-flight request should be resolved with empty vector.
  EXPECT_EQ(std::vector<double>{}, future.Get());
}
...
TEST_F(LocalAIServiceTest, InitFailureDrainsPendingRequests) {
  fake_model_worker_.set_init_success(false);
  ...
  // Init failed — pending request should be drained with empty result.
  EXPECT_EQ(std::vector<double>{}, future.Get());
  EXPECT_EQ(1, fake_model_worker_.init_count());
}
